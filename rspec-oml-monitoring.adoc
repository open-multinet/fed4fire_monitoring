...
- An optional +type+ attribute. This can be set to the type of the monitoring server, for example "zabbix".
...
- An optional +type+ attribute. *TODO: David introduced this to make it more simple to interpret it, but I think we can assume that upper level in the XML defines this already (this is node, link or general level/type)*. 
